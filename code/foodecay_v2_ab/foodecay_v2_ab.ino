...
// ------------------- Timings -------------------
#define SAMPLE_INTERVAL_MS       1000UL   // one reading every 30 s -> one file
...
// MQ-2 as "ethylene-like" hydrocarbon channel (approximate):
static inline float mq2EthylenePPM(float rs_over_r0) {
  if (rs_over_r0 <= 0.0f) return 0.0f;
  // Tuned so Rs/R0 ~ 1 ≈ 50 ppm; exponent from typical MQ-2 hydrocarbon slopes
...
// MQ-4 methane (from common CH4 fit ~ 1000 * (Rs/R0)^-2.95)
...
static void fillDataRecordOnce(DataRecord &rec) {
  ...
  //rec.mq4       = readADCavg(MQ4_A_Pin);
    // --- MQ2/MQ4: convert averaged ADC -> voltage -> Rs -> Rs/R0 -> ppm ---
...
void loop() {
  ...
  if (now - lastSample >= SAMPLE_INTERVAL_MS) {
    ...
    if (!isFileSystemMountSuccess) {
      ...
      // Create/write the single-record .bin file
      File f = SD.open(dataFilename, FILE_WRITE);
      if (f) {
